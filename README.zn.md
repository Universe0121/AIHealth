...
| 本地存储 | Room 2.6（单一数据库统一管理用户 / 诊断 / 药品 / 饮食 / 运动） |
...
│   └── src/main/
│       ├── java/com/aihealth/
│       │   ├── data/               # 数据层（唯一 Room 数据库 + DAO + 实体 + 模型）
│       │   │   ├── db/             # AppDatabase 与类型转换器
│       │   │   ├── dao/            # 药品 / 诊断 / 饮食 / 用户 DAO
│       │   │   ├── entity/         # Room 实体（Drug/Diagnosis/Diet/Sport/User）
│       │   │   └── model/          # 结构化诊断、状态统计等数据模型
│       │   ├── network/            # 百度 AI 服务、食材识别封装
│       │   ├── receiver/           # 用药提醒广播接收器
│       │   ├── ui/                 # 界面层
│       │   │   ├── activity/       # Activity（登录 / 主界面 / 相机 / 可视化等）
│       │   │   ├── fragment/       # 五大功能模块页面
│       │   │   ├── adapter/        # RecyclerView 适配器
│       │   │   ├── widget/         # 自定义图表、健康评分视图
│       │   │   └── model/          # 界面层数据模型
│       │   ├── util/               # OCR、诊断解析、权限、图像等工具
│       │   └── AiHealthApplication.java   # 全局 Application
│       └── res/                    # 布局 / 资源 / 主题 / 菜单
