...
import com.oppo.AIHealth.data.DietDao;
...
public class VisualizationFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        // 初始化数据库
        dbA = AppDatabaseA.getInstance(requireContext());
        dbC = AppDatabaseC.getInstance(requireContext());
    ...
    private void loadDataAndUpdateCharts() {
        Executors.newSingleThreadExecutor().execute(() -> {
            try {
                ...
                activeDrugCount = dbA.appDao().getActiveDrugCount();

                // 获取运动记录
                sportCount = dbA.appDao().getSportRecordCount();
